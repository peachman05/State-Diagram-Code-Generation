...
| DESCRIPTION:      
******************************************************************************/
void XcpCommand(uint8_t* pCommand )
{

    tXcpCto* pCmd = (tXcpCto*) pCommand;
    vuint8 err;
...
//        xcp.CrmLen = CRM_CONNECT_NMODE_LEN;
//
...
//        goto positive_response;
    ...
    if(CRO_CONNECT_MODE == NORMAL_MODE)  {
        ...
        xcp.CrmLen = CRM_CONNECT_UMODE_LEN;
        CRM_CONNECT_BB_ID = BB_ID;
        CRM_CONNECT_BB_SID = BB_ID;
        CRM_CONNECT_IOB_ID = IOB_ID;
        CRM_CONNECT_IOB_SID = IOB_SID;
        CRM_CONNECT_FW_ID = FW_ID;
        CRM_CONNECT_FW_SID = FW_SID;
        CRM_CONNECT_FPGA_ID = FPGA_ID;
        CRM_CONNECT_FPGA_SID = FPGA_SID;
        CRM_CONNECT_RESOUCE = RM_CAL_PAG;
          ...
          case CC_SHORT_DOWNLOAD :
          {
              vuint8 size;
                ...
                size = CRO_SHORT_DOWNLOAD_SIZE;
                  ...
                  if (size>CRO_SHORT_DOWNLOAD_MAX_SIZE) {

                    error(CRC_OUT_OF_RANGE)
                  }
              vuint8 * address;

              address = ApplXcpGetPointer(CRO_SHORT_DOWNLOAD_EXT,CRO_SHORT_DOWNLOAD_ADDR);

              if((address < (vuint8 *) BEGIN_XCP_ADDRESS) | (address > (vuint8 *) END_XCP_ADDRESS)) {

                  error(CRC_OUT_OF_RANGE)
              }
              else {

                  XcpSetMta(address,CRO_SHORT_DOWNLOAD_EXT);
...
//                  }
              ...
              address = ApplXcpGetPointer(CRO_SHORT_UPLOAD_EXT,CRO_SHORT_UPLOAD_ADDR);
              ...
              if((address < (vuint8 *) BEGIN_XCP_ADDRESS) | (address > (vuint8 *) END_XCP_ADDRESS)) {
                  ...
                  XcpSetMta(address,CRO_SHORT_UPLOAD_EXT);
                  err = XcpReadMta(CRO_SHORT_UPLOAD_SIZE,CRM_SHORT_UPLOAD_DATA);
                  xcp.CrmLen = (vuint8)((CRM_SHORT_UPLOAD_LEN+CRO_SHORT_UPLOAD_SIZE)&0xFFu);
                  if (err==(vuint8)XCP_CMD_PENDING)
                  {
...
//                  error(CRC_CMD_SYNTAX)
//                }
                err = ApplXcpUserService((MEMORY_ROM BYTEPTR) &CRO_BYTE(1),&xcp.Crm.b[1]);
                ...
                if(CRO_BYTE(1) == 1) {
                    ...
                    xcp.CrmLen = CRM_USER_SERVICE_SIZE;
                }
                else {
                    xcp.CrmLen = CRM_USER_SERVICE_SIZE+CRO_BYTE(2);
                }
/*End ThaiGerTec 2015*/
  #endif
              }
