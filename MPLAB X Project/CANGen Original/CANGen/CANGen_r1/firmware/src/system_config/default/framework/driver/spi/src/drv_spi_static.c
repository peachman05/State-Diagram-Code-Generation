...
void DRV_SPI0_Initialize(void)
{
    ...
    PLIB_SPI_SlaveSelectDisable(SPI_ID_2);
    ...
    PLIB_SPI_BaudRateSet(SPI_ID_2, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 500);
...
void DRV_SPI1_Initialize(void)
{
    ...
    PLIB_SPI_SlaveSelectDisable(SPI_ID_3);
    ...
    PLIB_SPI_FIFODisable( SPI_ID_3  );
    PLIB_SPI_BaudRateSet(SPI_ID_3, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 500);
...
int32_t DRV_SPI1_BufferAddWriteRead(const void * txBuffer, void * rxBuffer, uint32_t size)
{
    ...
    do {
        continueLoop = false;
        if(!PLIB_SPI_TransmitBufferIsFull(SPI_ID_3) && txcounter < size)
        {
        ...
        while (txcounter != rxcounter)
        {
            while(!PLIB_SPI_ReceiverBufferIsFull(SPI_ID_3));
